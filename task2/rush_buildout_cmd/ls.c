...
void fileDetails(char* file, struct stat * file_info);
...
void lsFunc(char* file, int mode)
{
    ...
    if (mode == ARGV_PLAIN) {
      ...
      if((stat(dirent_ptr->d_name, &file_info)) == -1) {
        printf("\n\e[1merror: ls: %s: %s.\e[0m\n", dirent_ptr->d_name, strerror(errno));
      } else {
        fileDetails(dirent_ptr->d_name, &file_info);
      }
    }
	}
	closedir(dir_ptr);
...
void permissionRights(char *str, int mode)
{
	...
	str[0] = S_ISBLK(mode) ? 'b' : '-'; //block device
