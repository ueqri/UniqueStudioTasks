...
int operatorHandleRedirect(char **args,int begin, int end) {
	...
	if (pid == -1) {
		...
		//need to check whether these two functions have run successfully.
		// if (in_file != NULL) fileIOInRedirect(in_file);
		// if (out_file != NULL) fileIOOutRedirect(out_file);
		if (in_file != NULL) {
			if (fileIOInRedirect(in_file) != FILE_IO_REDIRECT_SUCCESS) {
				exit(0);
			}
		}
		if (out_file != NULL) {
			if (fileIOOutRedirect(out_file) != FILE_IO_REDIRECT_SUCCESS) {
				exit(0);
			}
		}
